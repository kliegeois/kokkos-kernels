...
struct TeamVectorGemvInternal {
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(
      ...
      const int /*as1*/, const int /*as2*/, const ValueType *KOKKOS_RESTRICT /*x*/, const int /*xs0*/,
...
TeamVectorGemvInternal<Algo::Gemv::Unblocked>::invoke(
    ...
    const ValueType *KOKKOS_RESTRICT A, const int as0, const int as1, const int as2,
  ...
  if (beta == zero)
    Kokkos::parallel_for(
        Kokkos::TeamVectorRange(member, 0, N * m),
        [&](const int& iTemp) {
          int iRow, iMatrix;
          getIndices<int, layout>(iTemp, m, N, iRow,
                                          iMatrix);
    ...
    Kokkos::parallel_for(
        ...
        [&](const int& iTemp) {
          ...
          getIndices<int, layout>(iTemp, m, N, iRow,
                                          iMatrix);
          for (int i = 0; i < n; ++i)
            t += A[as0 * iMatrix + as1 * iRow + as2 * i] * X[xs0 * iMatrix + xs1 * i];
