...
CPU_time_l = np.zeros((n_implementations, len(Bs), n_quantiles))
CPU_time_r = np.zeros((n_implementations, len(Bs), n_quantiles))
throughput_l = np.zeros((n_implementations, len(Bs), n_quantiles))
throughput_r = np.zeros((n_implementations, len(Bs), n_quantiles))
...
for i in range(0, n_implementations):
    if i < 5:
        plot_quantiles(Bs, CPU_time_l[i,:,:], ax, i_skip=0, label='Impl '+str(i) + ' left')
        plot_quantiles(Bs, CPU_time_r[i,:,:], ax, i_skip=0, label='Impl '+str(i) + ' right')
    else:
        plot_quantiles(Bs, CPU_time_l[i,:,:], ax, i_skip=0, label='Impl '+str(i-5) + ' left with cache', dashed=True)
        plot_quantiles(Bs, CPU_time_r[i,:,:], ax, i_skip=0, label='Impl '+str(i-5) + ' right with cache', dashed=True)
...
for i in range(0, n_implementations):
    if i < 5:
        plot_quantiles(Bs, throughput_l[i,:,:], ax, i_skip=0, label='Impl '+str(i) + ' left')
        plot_quantiles(Bs, throughput_r[i,:,:], ax, i_skip=0, label='Impl '+str(i) + ' right')
    elif i < 10:
        plot_quantiles(Bs, throughput_l[i,:,:], ax, i_skip=0, label='Impl '+str(i-5) + ' left with cache', dashed=True)
        plot_quantiles(Bs, throughput_r[i,:,:], ax, i_skip=0, label='Impl '+str(i-5) + ' right with cache', dashed=True)
    else:
        plot_quantiles(Bs, throughput_l[i,:,:], ax, i_skip=0, label='Impl '+str(i-5) + ' left with global cache', dashed=True)
        plot_quantiles(Bs, throughput_r[i,:,:], ax, i_skip=0, label='Impl '+str(i-5) + ' right with global cache', dashed=True)
