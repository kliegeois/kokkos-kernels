...
def plot_results(base, function_of_N, implementations_CG, implementations_GMRES):
    ...
    CPU_time_sp = np.loadtxt(base+'CPU_time_0_sp.txt')[:,3]
    ...
    if print_cusolver_sparse:
        indices = np.argwhere(CPU_time_sp != 0.)
        plt.semilogy(x[indices], CPU_time_sp[indices], label='cusolver sparse', color=colourWheel[j%len(colourWheel)],
            linestyle = '-',
