...
for ortho_strategy in [0, 1]:
    for arnoldi_level in [0, 1]:
        for other_level in [0, 1]:
            if arnoldi_level == 0 and other_level == 1:
                continue
            base = 'Pele_pGMRES_'+specie+'_data_Scaled_Jacobi_7_'+str(ortho_strategy)+'_'+str(arnoldi_level)+'_'+str(other_level)+'_sorted/'
            ...
            print(str(ortho_strategy)+'_'+str(arnoldi_level)+'_'+str(other_level))
            fig = plt.figure()
            ax = plt.gca()
            for i in range(0, n_implementations):
                if i < 5:
