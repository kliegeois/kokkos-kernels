...
def combine_data_frame_from_files(filenames, avg_names):
    df0 = get_data_frame_from_file(filenames[0])
    df0 = df0.rename(columns={"Avg": avg_names[0]})
    df0 = df0.drop(["Min", "Max"], axis=1)
    for i in range(1, len(filenames)):
        df = get_data_frame_from_file(filenames[i])
...
def main():
    ...
    for i in range(0, len(Bs)):
        ...
        filenames.append('SPMV_NVPROF_data_1/nvprof.3.'+str(Bs[i])+'.'+str(nnzs)+'.0.51200.left.txt')
        avg_names.append(str(Bs[i]))

    df0 = combine_data_frame_from_files(filenames, avg_names)
    df0.to_csv('test.csv')
