...
def objectives(point):
    exec_name = point['exec_name']
    A_file_name = point['A_file_name']
    B_file_name = point['B_file_name']
    X_file_name = point['X_file_name']
    timer_filename = point['timer_filename']
...
def main():
    ...
    exec_names = [directory+'/KokkosBatched_Test_GMRES']
    Anames = [data_d+'/A.mm']
    Bnames = [data_d+'/B.mm']
    Xnames = [data_d+'/X']
    timernames = [data_d+'/timers']
    
    exec_name    = Categoricalnorm (exec_names, transform="onehot", name="exec_name")
    A_file_name    = Categoricalnorm (Anames, transform="onehot", name="A_file_name")
    B_file_name    = Categoricalnorm (Bnames, transform="onehot", name="B_file_name")
    X_file_name    = Categoricalnorm (Xnames, transform="onehot", name="X_file_name")
    timer_filename    = Categoricalnorm (timernames, transform="onehot", name="timer_filename")
    

    team_size = Integer     (1, 24, transform="normalize", name="team_size")
    vector_length = Integer     (1, 8, transform="normalize", name="vector_length")
    N_team = Integer     (1, 32, transform="normalize", name="N_team")

    input_space = Space([exec_name, A_file_name, B_file_name, X_file_name, timer_filename])
    parameter_space = Space([team_size, vector_length, N_team])
