...
struct Functor_Test_BatchedDenseCuSolve {
  ...
  Functor_Test_BatchedDenseCuSolve(const MatrixViewType &A, const VectorViewType &X,
  ...
  inline double run() {
    ...
    if (status2 != CUSOLVER_STATUS_SUCCESS) {
      if (status2 == CUSOLVER_STATUS_NOT_INITIALIZED)
        std::cout << "Error in cusolverDnDpotrsBatched with batchSize = " << batchSize << " and m = " << m  << " CUSOLVER_STATUS_NOT_INITIALIZED " << std::endl;
      if (status2 == CUSOLVER_STATUS_INVALID_VALUE)
        std::cout << "Error in cusolverDnDpotrsBatched with batchSize = " << batchSize << " and m = " << m  << " CUSOLVER_STATUS_INVALID_VALUE " << std::endl;
      if (status2 == CUSOLVER_STATUS_INTERNAL_ERROR)
        std::cout << "Error in cusolverDnDpotrsBatched with batchSize = " << batchSize << " and m = " << m  << " CUSOLVER_STATUS_INTERNAL_ERROR " << std::endl;
      cudaDeviceSynchronize();
      exec_space().fence();
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    for (auto i_impl : impls) {
      ...
      if (i_impl == 0) {
        if (layout_right) {
          t_spmv =
            Functor_Test_BatchedDenseCuSolve<exec_space,AMatrixViewLR,XYTypeLR>(
                aLR, xLR, yLR)
                .run();
        }
      }
