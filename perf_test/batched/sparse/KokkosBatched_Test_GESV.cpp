...
struct Functor_TestBatchedTeamGESV {
  ...
  Functor_TestBatchedTeamGESV(const MatrixViewType &A, const VectorViewType &X,
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const MemberType &member) const {
    ...
    KokkosBatched::TeamGesv<MemberType, KokkosBatched::Gesv::NoPivoting>::invoke(member, A, b, x);
  }
  ...
  inline double run() {
    ...
    using MatrixType =
        Kokkos::View<typename MatrixViewType::non_const_value_type **,
                     typename MatrixViewType::array_layout,
                     typename MatrixViewType::execution_space>;

    const int n    = _A.extent(1);
    size_t bytes_0 = MatrixType::shmem_size(n, n + 4);
...
struct Functor_TestBatchedTeamVectorGESV {
  ...
  KOKKOS_INLINE_FUNCTION
  Functor_TestBatchedTeamVectorGESV(const MatrixViewType &A, const VectorViewType &X,
                               const VectorViewType &B)
      : _A(A),
        _X(X),
        _B(B) {}
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const MemberType &member) const {
    ...
    KokkosBatched::TeamVectorGesv<MemberType, KokkosBatched::Gesv::NoPivoting>::invoke(member, A, b, x);
  }

  inline double run() {
    std::string name("KokkosBatched::Test::TeamVectorGESV");
    Kokkos::Timer timer;
    Kokkos::Profiling::pushRegion(name.c_str());
    Kokkos::TeamPolicy<DeviceType> policy(_A.extent(0), Kokkos::AUTO(), Kokkos::AUTO());
...
int main(int argc, char *argv[]) {
  Kokkos::initialize(argc, argv);
  {
    ...
    printf(" :::: Testing (N = %d, Blk = %d, vl = %d, n = %d)\n", N,
      ...
      for (int i_rep = 0; i_rep < n_rep_1 + n_skip; ++i_rep) {
        ...
        for (int j_rep = 0; j_rep < n_rep_2; ++j_rep) {
          ...
          if (i_impl == 0) {
            if (layout_left) {
              t_spmv +=
                Functor_TestBatchedTeamGESV<exec_space,AMatrixViewLL,XYTypeLL>(
                    aLL, xLL, yLL)
                    .run();
            }
            if (layout_right) {
              t_spmv +=
                Functor_TestBatchedTeamGESV<exec_space,AMatrixViewLR,XYTypeLR>(
                    aLR, xLR, yLR)
                    .run();
            }
          }
          if (i_impl == 1) {
            if (layout_left) {
              t_spmv +=
                Functor_TestBatchedTeamVectorGESV<exec_space,AMatrixViewLL,XYTypeLL>(
                    aLL, xLL, yLL)
                    .run();
            }
            if (layout_right) {
              t_spmv +=
                Functor_TestBatchedTeamVectorGESV<exec_space,AMatrixViewLR,XYTypeLR>(
                    aLR, xLR, yLR)
                    .run();
            }
          }
