...
void impl_test_batched_gesv(const int N, const int BlkSize) {
  ...
  Functor_TestBatchedTeamVectorGesv<DeviceType, MatrixType, VectorType>(A, X, B).run();
...
int test_batched_teamvector_gesv() {
#if defined(KOKKOSKERNELS_INST_LAYOUTLEFT)
  {
    ...
    for (int i = 3; i < 10; ++i) {
      Test::TeamVectorGesv::impl_test_batched_gesv<DeviceType, MatrixType, VectorType>(
          1024, i);
    }
  }
#endif
