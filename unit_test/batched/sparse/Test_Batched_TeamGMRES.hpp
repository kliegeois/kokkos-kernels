...
struct Functor_TestBatchedTeamGMRES {
  ...
  Functor_TestBatchedTeamGMRES(const ValuesViewType &D, const IntView &r,
      ...
      : _D(D), _r(r), _c(c), _X(X), _B(B), _N_team(N_team), _Diag(diag), _handle(handle) {
  }
  ...
  inline void run() {
    ...
    _handle.Arnoldi_view = typename KrylovHandleType::ArnoldiViewType("", N, maximum_iteration, n+maximum_iteration+3);
...
int test_batched_team_GMRES() {
#if defined(KOKKOSKERNELS_INST_LAYOUTLEFT)
  {
    ...
    for (int i = 3; i < 10; ++i) {
      Test::TeamGMRES::impl_test_batched_GMRES<DeviceType, ViewType,
